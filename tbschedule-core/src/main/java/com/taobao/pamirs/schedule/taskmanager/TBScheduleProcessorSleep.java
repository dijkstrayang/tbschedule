...
class TBScheduleProcessorSleep<T> implements IScheduleProcessor, Runnable
{
			...
			StatisticsInfo aStatisticsInfo)
	{
		...
		// 如果队列处理器是单个任务的，每次处理的数据量为1，设置批处理为false
	...
	protected int loadScheduleData()
	{
		try
		{
			...
			if (taskItems.size() > 0)
			{
				...
				List<T> tmpList = this.taskDealBean.selectTasks(taskTypeInfo.getTaskParameter(),
						scheduleManager.getScheduleServer().getOwnSign(), this.scheduleManager.getTaskItemCount(),
