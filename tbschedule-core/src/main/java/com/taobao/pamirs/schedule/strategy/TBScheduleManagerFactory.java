...
public class TBScheduleManagerFactory implements ApplicationContextAware
{
	...
	/**
	 * 调度服务器uuid ip$hostname$UUID$sequence
	 */
	private String uuid;
	private String ip;
	...
	public void reRegisterManagerFactory() throws Exception
	{
		...
		//根据策略重新分配调度任务机器的任务数，并在zk上更新对应的ScheduleStrategyRunntime中的AssignNum（分担的线程组数量？）
	...
	public void assignScheduleServer() throws Exception
	{
				...
				.loadAllScheduleStrategyRunntimeByUUID(this.uuid))
		{
			...
			//根据配置的任务项，进行线程组分配。传入参数：ManagerFactory个数、最大线程组数目、单jvm最大线程组数，返回nums：存储每个调度服务器上 用于的线程组的数量
			...
			for (int i = 0; i < factoryList.size(); i++)
			{
				...
				// 更新请求的服务器数量，将分配后结果写入到ZK中，等待其他ManagerFactory轮询获取
				this.scheduleStrategyManager.updateStrategyRunntimeReqestNum(run.getStrategyName(), factory.getUuid(),
						nums[i]);
			}
		}
	}
    ...
    /**
     * 遍历当前调度服务器承担的调度策略
     * 针对每个调度策略，根据调度策略运行时信息，与当前调度服务器已分配的任务调度分配器数量比较，多停少增
     * @throws Exception
     */
	public void reRunScheduleServer() throws Exception
	{
		for (ScheduleStrategyRunntime run : this.scheduleStrategyManager.loadAllScheduleStrategyRunntimeByUUID(this.uuid))
		{
			...
			// 不足，从/rootPath/strategy/strategyName 获取调度策略信息，增加当前调度服务器的任务调度分配器
	...
	public void run()
	{
		...
		try
		{
			...
			while (facotry.zkManager.checkZookeeperState() == false)
			{
				...
				if (count % 50 == 0)
				{
					facotry.errorMessage = "Zookeeper connecting ......" + facotry.zkManager.getConnectStr()
